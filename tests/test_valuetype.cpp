...
// compareValues must order by numeric value. A bytewise compare gets this wrong
...
static void testCompareValuesIsNumeric() {
    ...
    SY_CHECK(compareValues(hi, lo, ValueType::Int32) > 0);
    SY_CHECK(compareValues(lo, hi, ValueType::Int32) < 0);
    ...
    SY_CHECK(compareValues(lo, hi, ValueType::Float32) < 0);

    // Equality.
    ValueFormat::parse("7", ValueType::Int64, lo);
    ValueFormat::parse("7", ValueType::Int64, hi);
    SY_CHECK_EQ(compareValues(lo, hi, ValueType::Int64), 0);
}
